...
module SlowPartBody()
{
    union()
    {
        translate([0,0,SlowPartHigh/2]) 
            cylinder(r=D/2, h=bbT, center=true, $fn=100);
        cylinder($fa=12, $fs=2, h=SlowPartHigh-bbT, r1=slowPartSmallDiameter/2, r2=D/2, center=true, $fn=100);
    }
}
    ...
    translate([0,0,+SlowPartHigh/2])
        rotate([0,0,i*360/m+phi]) 
            translate([pitchD/2*(ns+np)/nr,0,0])
                cylinder(r=bbED/2 + adjustment, h=bbT, center=true, $fn=50);
}
...
module ThrustBallBearingHole() 
    translate([0,0,SlowPartHigh/2 - tbbT/2]) 
        cylinder(r=tbbED/2 + adjustment, h=tbbT+washers, $fn=50);  
